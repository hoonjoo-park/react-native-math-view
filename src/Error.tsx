...
export const ErrorComponent = (props: MathViewErrorProps) => (
  <View style={[props.style, styles.multilineText]}>
    <Text style={props.style}>{props.math}</Text>
    <Text style={[props.style, styles.error]}>{props.error}</Text>
  </View>
);

export const MathErrorBoundary = React.memo(
  (props: MathViewProps & { error: Error }) => {
    const { error, renderError: Fallback, onError } = props;
    if (_.every(_.values(MathError), (enumo) => enumo !== error.name))
      throw error;
    useEffect(() => {
      if (onError) {
        onError(error);
