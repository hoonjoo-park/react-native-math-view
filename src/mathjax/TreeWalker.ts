...
export type TreeWalkerCallback<N, T> = (
  node: N,
  level: number,
  accum: T[],
  quit: () => void
...
export function walkUp<
  N extends Child | LiteElement = LiteElement,
  T = LiteElement
>(node: N, callback: TreeWalkerCallback<N, T>) {
  let n: N = node;
  let i = 0;
  let _break = false;
  const quit = () => {
...
export function walkDown<
  N extends Parent | LiteElement = LiteElement,
  T = LiteElement
>(node: N, callback: TreeWalkerCallback<N, T>) {
  let stack = [] as N[];
  ...
  while (n) {
    ...
    if (_.size(children) > 0) {
      children = _.map(children, (child: N) =>
        _.set(child, "attributes.pLevel", i + 1)
      );
