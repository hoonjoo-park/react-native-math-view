...
class ConvertMemoize extends Memoize {
  ...
  covert(
    ...
    if (_.isNil(cached)) {
      const mathElement = doc.convert(
        math,
        ConvertMemoize.getConvertOptions(options)
      ) as LiteElement;
  ...
  get(math: string, options: MathToSVGConfig) {
    const item = _.find(
      this.cache,
      (item) => _.isEqual(math, item.math) && _.isEqual(options, item.options)
    );
...
export default class MathjaxAdaptor {
  ...
  constructor(options: MathToSVGConfig) {
    ...
    const strokeWidthStyles = _.pickBy(this.styles, (css, key) =>
      _.has(css, "stroke-width")
    );
    this.styleQuery = _.flatten(
      _.map(strokeWidthStyles, (css, key: string) => {
        const selectors = key.split(",");
        return _.map(selectors, (selector) => {
          const tree = TreeWalker.walkDown<
            TreeWalker.Parent<{ tagName: string; properties: any }>,
            LiteElement
          >(
            ...
            (n, l) =>
              new LiteElement(
                n.tagName,
                _.mapKeys(n.properties, (value, key) => _.kebabCase(key))
              )
          );
          return {
            tree: tree.reverse(),
            value: parseFloat(css["stroke-width"]),
          };
        });
      })
    );
  ...
  parseSVG(svgNode: LiteElement, includeStyle: boolean = false) {
    ...
    if (includeStyle) {
      ...
      const defs = `<defs><style>${this.css}</style>${this.adaptor.innerHTML(
        defsNode
      )}</defs>`;
      const closingTag = "</defs>";
      return (
        svg.slice(0, svg.indexOf("<defs")) +
        defs +
        svg.slice(svg.indexOf(closingTag) + closingTag.length)
      );
    } else {
      return svg;
    }
  ...
  private applyCSSRules(node: LiteElement) {
    TreeWalker.walkDown(node, (n: LiteElement) => {
      _.forEach(this.styleQuery, ({ tree, value }) => {
        const cssRuleCompliant = TreeWalker.walkUp<LiteElement, boolean>(
          n,
          (n, l, acc, quit) => {
            if (tree.length <= l) {
              quit();
            ...
            const isSameKind =
              ...
              (n.kind === "use" &&
                mirror.kind ===
                  this.adaptor.elementById(
                    node,
                    n.attributes["xlink:href"].slice(1)
                  ).kind);
            const complaint =
              isSameKind &&
  ...
  toSVG = _.memoize((math: string) => {
    ...
    const width = parseSize(
      this.adaptor.getAttribute(svgNode, "width"),
      this.options
    );
    const height = parseSize(
      this.adaptor.getAttribute(svgNode, "height"),
      this.options
    );
  ...
  toSVGArray(math: string) {
    ...
    const viewBox = _.map(
      _.split(svgNode.attributes["viewBox"], " "),
      parseFloat
    );
    ...
    const collection = TreeWalker.walkDown(clone, (n, l) => {
      ...
      const mat = compose(
        transformationToMatrix(n.parent, tempKey),
        transformationToMatrix(n)
      );
    ...
    const useCollection = _.filter(collection, (n) => {
      if (!TreeWalker.reject(n) && n.kind === "use") {
        //n.children.length === 0 && n
    ...
    return _.zipWith(responseArr, viewBoxes, (res, viewBox) =>
      _.assign(res, { viewBox })
    ) as MathFragmentResponse[];
  }
  ...
  preload(mathArray: string[]) {
    if (_.size(mathArray) > 0) {
      setTimeout(() => {
        ...
        if (__DEV__)
          console.log("react-native-math-view: Mathjax preload completed");
      }, 0);
    }
  }
}
