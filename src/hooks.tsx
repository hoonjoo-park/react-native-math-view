...
export function useAsyncParser(props: MathViewProps) {
  ...
  const [result, setResult] = useState<
    ParserResponse | { error: Error } | undefined
  >(() =>
...
export function mathViewAsyncRenderer<
  T extends MathViewInjectedProps,
  R extends any
>(render: React.ForwardRefRenderFunction<R, T>) {
...
export function mathViewSyncRenderer<
  T extends MathViewInjectedProps,
  R extends any
>(render: React.ForwardRefRenderFunction<R, T>) {
  return React.forwardRef((props: MathViewProps, ref: React.Ref<R>) => {
...
export function mathViewRender<T extends MathViewInjectedProps, R extends any>(
  render: React.ForwardRefRenderFunction<R, T>,
  options: { async?: boolean } = {}
) {
  return options.async
    ? mathViewAsyncRenderer(render)
    : mathViewSyncRenderer(render);
}
